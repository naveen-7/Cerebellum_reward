...
if strcmp(response,'N')
    disp('enter the itemsthat belong to the same group')
...
for i=1:size(UNI,1)
        h(i) = subplot(3,3,i);
        hold on;
        plot(WAVES(IND==i,:)','color',[0.7 0.7 0.7]);
        plot(nanmean(WAVES(IND==i,:)),'color',[1 0 0]);
end
    linkaxes(h,'xy');
cd(codes_dir);
