...
% function CER_analysis_01_ADAPTED_MISMATCHLL_n
...
disp('!!!!!  CER_analysis_01_ADAPTED_MISMATCHLL_n has started running  !!!!!')
...
if  isempty(UNIQUE_CODES)
    ...
    if max(UNIQUE_CODES)==1 
        disp(strcat('------There is only ONE Lick Code available--------'));
        disp('>>> Proceeding >>>');
        CODE_S = 1;
        max_sspikes   = max(UNIQUE_CODES);
